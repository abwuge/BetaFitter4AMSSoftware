...
std::vector<ParticleData> Util::loadParticleData(const std::string &inputFile)
{
    ...
    tree->SetBranchAddress("mcCoo", data.mcCoo); // MC initial position
    tree->SetBranchAddress("mcDir", data.mcDir); // MC initial direction
...
std::vector<ParticleData> Util::loadParticleData2(const std::string &inputFile)
{
    std::vector<ParticleData> particles;
