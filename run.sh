...
if [[ $# -lt 2 ]]; then
    ...
    echo "Usage: $0 inputFile outputFile [fitOption=0] [energyLossScale=1.0] [trackerEnergyLossScale=1.0] [energyLossScaleMode=all|s1s2|s2] [referencePoint=center|before_tof] [scaleConfig=none|path]"
    exit 1
fi
...
"${SCRIPT_DIR}/build/bin/betaFitter" "$inputFile" "$outputFile" "$fitOption" \
    "$energyLossScale" "$trackerEnergyLossScale" "$energyLossScaleMode" "$referencePoint" "$scaleConfig"
